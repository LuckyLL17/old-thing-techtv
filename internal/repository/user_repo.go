...
func (r *UserRepo) UpdatePassword(id uint64, hash string) error {
	err := r.db.Model(&domain.User{}).Where("id = ?", id).Update("password_hash", hash).Update("updated_at", gorm.Expr("CURRENT_TIMESTAMP")).Error
	if err != nil {
...
func v6Task002Boundary2(left, right uint64) bool {
